...
exports.isValidSerial = isValidSerial;
exports.extractSerialComponents = extractSerialComponents;
exports.enforceMaxLength = enforceMaxLength;
exports.isFormattedSerial = isFormattedSerial;
...
var PREFIX_PATTERN = /^[0-9][A-Z]{2}$/; // 1 digit followed by 2 letters

var NUMERIC_PART_PATTERN = /^[0-9]{6}$/; // 6 digits

var FULL_SERIAL_PATTERN = /^[0-9][A-Z]{2}[0-9]{6}$/; // Complete serial number
...
function formatSerialNumber(serialNumber) {
  var cleaned = normalize(serialNumber);

  if (!cleaned || cleaned.length < 9) {
    return cleaned; // Return as-is if not complete but still valid
  }

  return "".concat(cleaned.slice(0, 3), " ").concat(cleaned.slice(3));
}
/**
...
function isValidSerial(serialNumber) {
  var cleaned = normalize(serialNumber);
  if (!cleaned) return false;
  var components = extractSerialComponents(cleaned);
  if (!components) return false;
  var prefix = components.prefix,
      numericPart = components.numericPart;
...
function extractSerialComponents(serialNumber) {
  var cleaned = normalize(serialNumber);

  if (!cleaned || cleaned.length < 9) {
    console.warn("Invalid serial number structure: \"".concat(serialNumber, "\""));
    return null;
  }
...
function enforceMaxLength(serialNumber) {
  var cleaned = normalize(serialNumber);
...
function isFormattedSerial(serialNumber) {
  var cleaned = normalize(serialNumber);
